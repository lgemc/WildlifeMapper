...
class SetCriterion(nn.Module):
    ...
    Id 57 is reserved for "no-object" or "Background" class
...
def _build_sam(
    ...
    sam = Sam(
        ...
        mask_decoder=MaskDecoder(
            num_multimask_outputs=50, #number of obects per image
    ...
    if checkpoint is not None:
        with open(checkpoint, "rb") as f:
            ...
            del_params = [p for p in mask_dec_params if 'transformer' not in p]
            #delete all other params since we have diff.
            for d_p in del_params:
                del state_dict[d_p]
