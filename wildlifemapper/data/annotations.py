...
class AnnotationsFromCSV(Annotations):
    ...
    def __init__(self, csv: Union[str,pandas.DataFrame], images_col: str = 'images', labels_col: str = 'labels') -> None:
        '''
        Args:
            csv (str or pandas.DataFrame): absolute path to the CSV file (with extension),
                or DataFrame object.
            images_col (str, optional): name of the column containing image names. Defaults to 'images'.
            labels_col (str, optional): name of the column containing labels. Defaults to 'labels'.
        '''
        ...
        assert {images_col, labels_col}.issubset(data_df.columns), \
            f'File must contain at least {images_col} and {labels_col} columns'

        images = list(data_df[images_col])
        labels = list(data_df[labels_col])
        annos = objects_from_df(data_df)

        # get other information
        supp_arg = {}
        for column, content in data_df.items():
            if column not in [images_col, labels_col] and column.startswith(('x','y')) is False:
